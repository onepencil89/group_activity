...
def get_exif_datetime(image):
    ...
    try:
        ...
        # DateTime, DateTimeOriginal, DateTimeDigitized 순으로 시도
...
def get_gps_from_image(image):
    \"\"\"
    이미지에서 GPS 좌표를 추출합니다.
    Returns: (latitude, longitude) 또는 (None, None)
    \"\"\"
    try:
        exif_data = image._getexif()
                        ...
                        with st.spinner(\"🤖 AI가 코스 위에서 유사한 사진을 찾고 있습니다...\"):
                            try:
                                ...
                                if len(results) == 0:
                                    ...
                                    st.success(f\"✅ **{len(results)}장**의 유사한 `
}