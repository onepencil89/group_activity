...
if mode == "📸 작가 모드":
    ...
    if uploaded and latlon:
        if st.button(f"💾 {len(uploaded)}장 DB에 저장하기", type="primary"):
            ...
            # --- 중복 방지 기준 생성 ---
            existing_photos = {
                (p["name"], round(p["lat"], 6), round(p["lon"], 6))
            ...
            for idx, f in enumerate(uploaded):
                ...
                lat_rounded, lon_rounded = round(latlon[0], 6), round(latlon[1], 6)
                if (f.name, lat_rounded, lon_rounded) in existing_photos:
                    st.warning(f"이미 저장된 사진: {f.name} (중복 저장 방지)")
                    continue
            ...
            for p in st.session_state["photos"]:
                ...
                if key not in unique:
                    unique[key] = p
            st.session_state["photos"] = list(unique.values())

            st.success(f"🎉 중복 없이 {len(uploaded)}장 업로드 완료!")
            progress_bar.empty()
